...
export function MetricCard({ icon: Icon, value, label, description, to, onClick }: MetricCardProps) {
  ...
  // Pretext measurement for string descriptions
  const descText = typeof description === "string" ? description : "";
  const { containerRef: descRef, style: descStyle } = usePretext(descText, {
    ...
    <div className={`h-full px-4 py-4 sm:px-5 sm:py-5 rounded-lg transition-colors${isClickable ? " hover:bg-accent/50 cursor-pointer" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {description && (
            <div
              ref={descText ? descRef : undefined}
              className="text-xs text-muted-foreground/70 mt-1.5 hidden sm:block"
              style={descText ? descStyle : undefined}
            >
              {description}
            </div>
          )}
